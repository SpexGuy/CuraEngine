...
class gcode(object):
    ...
    def calculateWeight(self):
        ...
        # this is volume * incorrect_density
        return volumeM3 * 1.0
    
    def calculateCost(self):
        cost_kg = 500 # in namibian rupies please 
        cost_meter = 0
    ...
    def _load(self, gcodeFile):
        ...
        for line in gcodeFile:
            ...
            if line.startswith(';COLOR_START'):
                if colorLength != 0:
                    self.colorList.append((color, colorLength))
                colorLength = 0
                color = tuple(float(val) for val in line[12:].strip().split())
            if line.startswith(';COLOR_STOP'):
                if colorLength != 0:
                    self.colorList.append((color, colorLength))
                colorLength = 0
                color = None
...
if __name__ == '__main__':
    ...
    print time.time() - t
    defaultColor = (1,1,1)
    for color in g.colorList:
        if not color[0]:
            color = (defaultColor, color[1])
        print "C1 R%0.3f G%0.3f B%0.3f E%0.6f" % (color[0][0], color[0][1], color[0][2], color[1])
